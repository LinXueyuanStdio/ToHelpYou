...
import com.tohelpyou.client.activity_fragment.GethelpListFragment;
import com.tohelpyou.client.activity_fragment.HelpListFragment;
import com.tohelpyou.client.activity_fragment.MineFragment;
import com.tohelpyou.client.activity_fragment.MomentListFragment;
import com.tohelpyou.client.activity_fragment.PublishFragment;
import com.tohelpyou.client.activity_fragment.UserListFragment;
import com.tohelpyou.client.application.ThyApplication;
import com.tohelpyou.client.base.BaseBottomTabActivity;
import com.tohelpyou.client.interfaces.TopBarMenuCallback;
import com.tohelpyou.client.model.User;
import com.tohelpyou.client.util.ActionUtil;
...
public class MainActivity extends BaseBottomTabActivity implements OnBottomDragListener {
	...
	protected int[] getTabClickIds() {
		return new int[]{R.id.llMainTabTab0, R.id.llMainTabTab1, R.id.llMainTabTab2, R.id.llMainTabTab3, R.id.llMainTabTab4};
	}

	@Override
	protected int[][] getTabSelectIds() {
		return new int[][]{
				new int[]{R.id.ivMainTabTab0, R.id.ivMainTabTab1, R.id.ivMainTabTab2, R.id.ivMainTabTab3, R.id.ivMainTabTab4},//顶部图标
				new int[]{R.id.tvMainTabTab0, R.id.tvMainTabTab1, R.id.tvMainTabTab2, R.id.tvMainTabTab3, R.id.tvMainTabTab4}//底部文字
		};
	}
...
//	return fragment;
	...
	protected Fragment getFragment(int position) {
		switch (position) {
		case 1:
			return GethelpListFragment.createInstance();
		case 2:
		...
		default:
			return HelpListFragment.createInstance();
		}
	}

	private static final String[] TAB_NAMES = {"帮", "派", "发表", "消息", "我的"};
	@Override
	...
	protected void reloadAll() {
		Log.d(TAG, "reloadAll >>> ");
		HttpRequest.getUser(ThyApplication.getInstance().getCurrentUserId(), 0, new OnHttpResponseListener() {

			@Override
			public void onHttpResponse(int requestCode, String resultJson, Exception e) {
				Log.d(TAG, "reloadAll >>> HttpRequest.getUser.onHttpResponse >>  saveCurrentUser >>");
				ThyApplication.getInstance().saveCurrentUser(new JSONResponse(resultJson).getObject(User.class));
				runUiThread(new Runnable() {

					@Override
					public void run() {
						sendBroadcast(new Intent(ActionUtil.ACTION_USER_CHANGED)
						.putExtra(INTENT_ID, ThyApplication.getInstance().getCurrentUserId())
						.putExtra(ActionUtil.INTENT_USER, ThyApplication.getInstance().getCurrentUser()));
					}
				});
			}
		});
