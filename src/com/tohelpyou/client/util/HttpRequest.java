...
package com.tohelpyou.client.util;

import java.util.ArrayList;
import java.util.List;

import com.tohelpyou.client.application.ThyApplication;
import com.tohelpyou.client.manager.HttpManager;
import com.tohelpyou.client.model.CommentItem;
import com.tohelpyou.server.model.Comment;
import com.tohelpyou.server.model.Login;
import com.tohelpyou.server.model.Moment;
import com.tohelpyou.server.model.Password;
import com.tohelpyou.server.model.User;
import com.tohelpyou.server.model.Wallet;
...
public class HttpRequest {
	...
	private static ThyApplication application;
	/**基础URL，这里服务器设置可切换*/
	public static String URL_BASE;
	static {
		application = ThyApplication.getInstance();
	...
	public static void logout(int requestCode, OnHttpResponseListener listener) {
		post(new JSONRequest(new Login(ThyApplication.getInstance().getCurrentUserId()).setType(0)
	...
	public static void setIsFriend(long id, boolean isFriend, int requestCode, OnHttpResponseListener listener) {
		User user = ThyApplication.getInstance().getCurrentUser();
	...
	public static void getUserList(int range, long id, com.alibaba.fastjson.JSONObject search, List<Long> idList
		...
		if (idList != null) {
			...
			com.tohelpyou.client.model.User currentUser = ThyApplication.getInstance().getCurrentUser();
			if (currentUser == null) {
				currentUser = new com.tohelpyou.client.model.User();
			}
			switch (range) {
			...
			case RANGE_USER_CIRCLE:
				if (ThyApplication.getInstance().isCurrentUser(id) == false) {
	...
	public static void praiseMoment(long id, boolean toPraise, int requestCode, OnHttpResponseListener listener) {
		...
		List<Long> list = new ArrayList<Long>();
		list.add(ThyApplication.getInstance().getCurrentUserId());
	...
	public static void getWallet(int requestCode, OnHttpResponseListener listener) {
		long userId = ThyApplication.getInstance().getCurrentUserId();
	...
	public static void changeBalance(double change, String payPassword, int requestCode, OnHttpResponseListener listener) {
		com.tohelpyou.client.model.User user = ThyApplication.getInstance().getCurrentUser();
		if (user == null) {
			user = new com.tohelpyou.client.model.User();
		}
