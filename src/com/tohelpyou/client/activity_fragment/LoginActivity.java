...
import com.tohelpyou.client.manager.DataManager;
...
public class LoginActivity extends BaseActivity implements OnClickListener, OnTouchListener, OnBottomDragListener{
		...
		HttpRequest.login(phone, password, type, type, new OnHttpResponseListener() {
			...
			public void onHttpResponse(int requestCode, String resultJson, Exception e) {
				...
				if (response.isSucceed() == false) {
					...
					ThyApplication.getInstance().saveCurrentUser(user);
					if (ThyApplication.getInstance().isLoggedIn() == false) {
	...
	public void finish() {
		...
		setResult(RESULT_OK, new Intent().putExtra(RESULT_LOGINED, ThyApplication.getInstance().isLoggedIn()));
		super.finish();
	}
