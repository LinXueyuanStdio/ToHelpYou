...
import com.tohelpyou.client.util.CommentUtil;
import com.tohelpyou.client.util.HttpRequest;
import com.tohelpyou.client.view.MomentView;
...
, OnBottomDragListener, OnClickListener, OnDialogButtonClickListener {
	...
	public void onCommentClick(CommentItem item, int position, int index, boolean isLong) {
		if (isLong) {
			...
			if (ThyApplication.getInstance().isCurrentUser(momentItem.getUserId()) == false
					&& ThyApplication.getInstance().isCurrentUser(item.getUserId()) == false) {
