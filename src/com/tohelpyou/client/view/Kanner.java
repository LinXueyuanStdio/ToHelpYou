...
public class Kanner extends FrameLayout {
	...
	private void initImgFromRes(int[] imagesRes) {
		...
		for (int i = 0; i < count; i++) {
			...
			iv_dot.setImageResource(R.drawable.kanner_dot_blur);
			ll_dot.addView(iv_dot, params);
			iv_dots.add(iv_dot);
		}
		iv_dots.get(0).setImageResource(R.drawable.kanner_dot_focus);
	...
	class MyOnPageChangeListener implements OnPageChangeListener {
		...
		public void onPageSelected(int arg0) {
			for (int i = 0; i < iv_dots.size(); i++) {
				if (i == arg0 - 1) {
					iv_dots.get(i).setImageResource(R.drawable.kanner_dot_focus);
				} else {
					iv_dots.get(i).setImageResource(R.drawable.kanner_dot_blur);
				}
			}
		}
